...
def decoding_attn_fwd(
    ...
    Arguments:
        ...
        k: [total_k, head_k, dim], torch.float16 / torch.bfloat16, where total_k = total number of 
            key tokens in the batch.
        v: [total_k, head_k, dim], torch.float16 / torch.bfloat16, where total_k = total number of 
            value tokens in the batch.
    ...
    Return:
        o: [total_q, head_q, dim], torch.float16 / torch.bfloat16, where total_q = total number of 
            output tokens in the batch.
    """
    q, k, v = [maybe_contiguous(x) for x in (q, k, v)]
    o = decoding_attn_cuda.fwd(
        q, k, v, None, cu_seq_k, max_seq_k, is_alibi)
    return o
