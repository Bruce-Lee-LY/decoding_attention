...
echo "========== build decoding_attention =========="

echo_cmd "rm -rf build output"
echo_cmd "mkdir build"

echo_cmd "cd build"
echo_cmd "cmake -DCMAKE_CUDA_ARCHITECTURES=$CUDA_ARCHITECTURE -DCMAKE_BUILD_TYPE=$BUILD_TYPE -DDA_VERBOSE_MAKEFILE=$VERBOSE_MAKEFILE -DCMAKE_INSTALL_PREFIX=$WORK_PATH/output -DCMAKE_SKIP_RPATH=ON .."
...
echo "branch: $BRANCH" >> $WORK_PATH/output/da_version
echo "commit: $COMMIT" >> $WORK_PATH/output/da_version
echo "gcc_version: $GCC_VERSION" >> $WORK_PATH/output/da_version
echo "compile_time: $COMPILE_TIME" >> $WORK_PATH/output/da_version
