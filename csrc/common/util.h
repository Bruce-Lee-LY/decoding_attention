...
inline int convert_SM_to_cores(int major, int minor) {
    ...
    DLOG("MapSMtoCores for SM %d.%d is undefined. Default to use %d cores/SM", major, minor,
...
inline const char *convert_SM_to_arch_name(int major, int minor) {
    ...
    // If we don't find the values, we default use the previous one to run properly
    DLOG("MapSMtoArchName for SM %d.%d is undefined. Default to use %s", major, minor, nGpuArchNameSM[index - 1].name);
