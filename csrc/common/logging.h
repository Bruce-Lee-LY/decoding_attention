...
#define DA_LOG_TAG "DA"
#define DA_LOG_FILE(x) (strrchr(x, '/') ? (strrchr(x, '/') + 1) : x)
#define DLOG(format, ...)                                                                                      \
    do {                                                                                                       \
        fprintf(stderr, "[%s %s %d:%ld %s:%d %s] " format "\n", DA_LOG_TAG, curr_time(), get_pid(), get_tid(), \
                DA_LOG_FILE(__FILE__), __LINE__, __FUNCTION__, ##__VA_ARGS__);                                 \
    } while (0)
