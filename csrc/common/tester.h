...
public:
    explicit Tester(size_t batch = 2, size_t seq_q = 1, size_t seq_k = 256, size_t head_q = 32, size_t head_k = 32,
        ...
        DA_CHECK(m_K);
        m_V = std::make_shared<Tensor<T>>(std::vector<size_t>{m_total_k, m_head_k, m_dim}, "Tensor V");
        DA_CHECK(m_V);
        m_O = std::make_shared<Tensor<T>>(std::vector<size_t>{m_total_q, m_head_q, m_dim}, "Tensor O");
        DA_CHECK(m_O);
        m_base = std::make_shared<Tensor<T>>(std::vector<size_t>{m_total_q, m_head_q, m_dim}, "Tensor Base");
        DA_CHECK(m_base);
        ...
        if (m_enable_check) {
            clock_t start = clock();
            mha_cpu(m_Q.get(), m_K.get(), m_V.get(), m_base.get(), m_cu_seq_k.get(), m_seq_k, m_is_causal, m_is_alibi);
            clock_t end = clock();
    ...
    void evaluate(Func &&dmha, const std::string &name) {
        ...
        for (size_t i = 0; i < m_warmup_iterations; ++i) {
            dmha(reinterpret_cast<void *>(m_Q->getDevPtr()), reinterpret_cast<void *>(m_K->getDevPtr()),
                 reinterpret_cast<void *>(m_V->getDevPtr()), reinterpret_cast<void *>(m_O->getDevPtr()),
                 m_cu_seq_k->getDevPtr(), m_seq_k, m_batch, m_head_q, m_head_k, m_dim, m_is_alibi, m_is_bf16, m_stream);
        }
...
private:
    ...
    void mha_cpu(Tensor<T> *Q, Tensor<T> *K, Tensor<T> *V, Tensor<T> *O, Tensor<int> *cu_seq_k, size_t max_seq_k,
                ...
                for (size_t sq = 0; sq < seq_q; ++sq) {
                    for (size_t d = 0; d < dim; ++d) {
                        ...
                        if constexpr (std::is_same_v<T, half>) {
                            ...
                            o_ptr[(sum_seq_q + sq) * (head_q * dim) + h * dim + d] = __float2bfloat16(acc);
                        }
                    }
                }
            }
    ...
    void profile(Func &&dmha, const std::string &name) {
        ...
        DLOG("%s exit, profiling time: %.4f ms (%.2f%%), throughput: %.4f TFLOPS (%.2f%%)", name.c_str(),
    ...
    std::shared_ptr<Tensor<T>> m_V = nullptr;     // total_k * head_k * dim
    std::shared_ptr<Tensor<T>> m_O = nullptr;     // total_q * head_q * dim
    std::shared_ptr<Tensor<T>> m_base = nullptr;  // total_q * head_q * dim, base result, init tensor O before each dmha
