...
void set_dmha_fwd_params(DecodingParams &params, void *q, void *k, void *v, void *o, int *cu_seq_k, size_t max_seq_k,
                         size_t batch, size_t head_q, size_t head_k, size_t dim, bool is_alibi, bool is_bf16,
                         cudaStream_t stream) {
    DA_CHECK(q);
    DA_CHECK(k);
    DA_CHECK(v);
...
void run_dmha_fwd(const DecodingParams &params) {
    DA_FP16_SWITCH(!params.is_bf16,
                   [&] { DA_HEADDIM_SWITCH(params.d, [&] { run_dmha_fwd_<elem_type, head_dim>(params); }); });
}
...
void decoding_attn(void *q, void *k, void *v, void *o, int *cu_seq_k, size_t max_seq_k, size_t batch, size_t head_q,
    ...
    set_dmha_fwd_params(params, q, k, v, o, cu_seq_k, max_seq_k, batch, head_q, head_k, dim, is_alibi, is_bf16, stream);
    run_dmha_fwd(params);
}
