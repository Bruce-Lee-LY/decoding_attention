...
void dmha_fwd(const DecodingParams &params) {
    ...
    const size_t dynamic_smem_size = std::max(params.max_seq_k * sizeof(float), params.d_v * sizeof(float));
    ...
    DA_BOOL_SWITCH(params.is_alibi, IsAlibi, [&] {
        auto kernel =
            &dmha_fwd_kernel<T, DecodingKernelTraits<T, HeadDim, HeadDimV, ThreadsPerBlock, ThreadsPerGroup>, IsAlibi>;
